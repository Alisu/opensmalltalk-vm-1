...
sqInt
ioSeconds() { return get64(localMicrosecondClock) / MicrosecondsPerSecond; }

sqInt
ioSecondsNow() { return ioLocalMicrosecondsNow() / MicrosecondsPerSecond; }
...
#if macintoshSqueak
sqInt
ioRelinquishProcessorForMicroseconds(sqInt microSeconds, struct foo * self)
{
    long	realTimeToWait;
	extern usqLong getNextWakeupUsecs();
	usqLong nextWakeupUsecs = getNextWakeupUsecs(self);
...
static void *
beatStateMachine(struct foo * self)
{
    int er;
...
void setIOHeartbeatGIVToUse(int anInteger){
...
void
ioInitHeartbeat(struct foo * self)
{
	int er;
